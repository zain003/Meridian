...
- [ ] `vitest run tests/unit/auth-workspace.test.ts` — Pass / Fail
...
- [ ] `vitest run tests/components/auth-forms.test.tsx` — Pass / Fail
...
- [ ] `npm run lint` passes with no lint warnings or errors.
- [ ] No hardcoded secrets or arbitrary color hex codes present in files.
- [ ] `DEVIATIONS.md` contains entries for any runtime assumptions made during implementation.
